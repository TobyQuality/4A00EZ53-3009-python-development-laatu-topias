...
def name_form():
    if request.method == 'POST':
        ...
        if is_name(name):
            ...
            return render_template('name-form.html', notification=f'{fname} {lname} added to database')
        else:
...
def database():
    ...
    if request.method == 'POST':
        ...
        if is_name(name):
            ...
            db_str = read_database()
            db_list = csv_to_list(db_str)
            return render_template("database.html", customers=db_list, firstname="", lastname="")
        else:
            return render_template("database.html", customers=db_list, notification="Adding customer to database failed", firstname="", lastname="")
